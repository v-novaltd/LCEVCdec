...
namespace {
    ...
    {
        constexpr PictureFormatInfo(uint32_t planeCount, uint32_t numChannels, uint8_t bitDepth,
        ...
        {
            VNAssertf(planeCount <= VN_IMAGE_NUM_PLANES,
                      "Invalid image desc, maximum plane count is %d", VN_IMAGE_NUM_PLANES);
        }
    ...
    constexpr PictureFormatInfo kFormatInfo[PictureFormat::Count] = {
...
PictureFormat::Enum PictureFormat::FromBitDepthChroma(BitDepthType::Enum depth, ChromaSamplingType::Enum chroma)
{
                                   ...
                                   const uint32_t planeStrideBytes[kMaxNumPlanes] /*= nullptr*/)
{
    ...
    m_colorspace = (colorspace == Colorspace::Auto) ? Colorspace::AutoDetectFromFormat(format) : colorspace;
    ...
    for (uint32_t planeIndex = 0; planeIndex < m_planeCount; ++planeIndex) {
        // Get various conversion factors.
        const uint32_t samplesPerPixel =
            ((planeIndex != 0) && (m_interleaving == PictureInterleaving::NV12)) ? 2 : 1;
        const uint32_t bytesPerPixel = bytesPerSample * samplesPerPixel;
        VNAssert(bytesPerPixel > 0);
        ...
        const uint32_t minimumBytesPerRow = (planeWidth * bytesPerPixel);
        const uint32_t bytesPerRow =
            (planeStrideBytes ? planeStrideBytes[planeIndex] : minimumBytesPerRow);
        if (bytesPerRow < minimumBytesPerRow) {
            // This is invalid: you can have junk "padding" pixels on the end, but you can't
            // display more pixels than your byte stride can hold.
            return false;
        }

        // We now know enough to create a plane desc and determine the total size.
        m_planeDesc[planeIndex] = PlaneDesc(planeWidth, planeHeight, bytesPerRow / bytesPerPixel,
                                            bytesPerRow, samplesPerPixel);
