...
public:
    HashXxHash() : m_state(XXH64_createState()) { XXH64_reset(m_state, 0); }
    ...
    void update(const uint8_t *data, uint32_t size)  final {
        XXH64_update(m_state, data, size);
    }

    std::string hexDigest() final {
        return fmt::format("{:016x}",XXH64_digest(m_state));
    }
...
void Hash::updatePicture(LCEVC_DecoderHandle decoder, LCEVC_PictureHandle picture)
{
    ...
    for (uint32_t plane = 0; plane < lock.numPlanes(); ++plane) {
        for (uint32_t row = 0; row < lock.planeDesc(plane).height; ++row) {
