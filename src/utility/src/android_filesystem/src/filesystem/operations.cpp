...
namespace { namespace parser {
    ...
    public:
        PathParser(string_view_t P, string_view_t E, unsigned char S)
            : Path(P)
            , RawEntry(E)
            , State(static_cast<ParserState>(S))
        {
        ...
        void increment() noexcept
        {
            ...
            switch (State) {
                ...
                case PS_InRootDir: return makeState(PS_InFilenames, Start, consumeName(Start, End));
                ...
                case PS_InTrailingSep: return makeState(PS_AtEnd);
        ...
        void decrement() noexcept
        {
            ...
            switch (State) {
                ...
                case PS_InRootName:
                case PS_BeforeBegin: _LIBCPP_UNREACHABLE();
            }
        }
        ...
        string_view_t operator*() const noexcept
        {
            switch (State) {
                case PS_BeforeBegin:
                case PS_AtEnd: return "";
                case PS_InRootDir: return "/";
                case PS_InTrailingSep: return "";
                case PS_InRootName:
                case PS_InFilenames: return RawEntry;
            }
            _LIBCPP_UNREACHABLE();
        }
    ...
    private:
        ...
        {
            switch (State) {
                case PS_BeforeBegin: return Path.data();
                case PS_InRootName:
                case PS_InRootDir:
                case PS_InFilenames: return &RawEntry.back() + 1;
                case PS_InTrailingSep:
                case PS_AtEnd: return getAfterBack();
            }
            _LIBCPP_UNREACHABLE();
        }
        ...
        PosPtr getCurrentTokenStartPos() const noexcept
        {
            switch (State) {
                case PS_BeforeBegin:
                case PS_InRootName: return &Path.front();
                case PS_InRootDir:
                case PS_InFilenames:
                case PS_InTrailingSep: return &RawEntry.front();
                case PS_AtEnd: return &Path.back() + 1;
            }
            _LIBCPP_UNREACHABLE();
        }
...
void filesystem_error::__create_what(int __num_paths)
{
    ...
    __storage_->__what_ = [&]() -> string {
        ...
        switch (__num_paths) {
            default: return detail::format_string("filesystem error: %s", derived_what);
            case 1: return detail::format_string("filesystem error: %s [%s]", derived_what, p1);
            case 2:
...
void __copy(const path& from, const path& to, copy_options options, error_code* ec)
{
    ...
    error_code m_ec1;
    StatT f_st = {};
    const file_status f = sym_status || sym_status2 ? detail::posix_lstat(from, f_st, &m_ec1)
    ...
    StatT t_st = {};
    const file_status t =
        sym_status ? detail::posix_lstat(to, t_st, &m_ec1) : detail::posix_stat(to, t_st, &m_ec1);
    ...
    if (is_symlink(f)) {
        ...
        return err.report(errc::is_a_directory);
               ...
               (bool(copy_options::recursive & options) || copy_options::none == options)) {
        if (!exists(t)) {
...
namespace detail { namespace {
    ...
    __attribute__((unused)) bool copy_file_impl_default(FileDescriptor& read_fd,
                                                        FileDescriptor& write_fd, error_code& ec)
    {
        ifstream in;
...
{
    static_assert(PathParser::PS_BeforeBegin == 1 && PathParser::PS_InRootName == 2,
...
path path::lexically_normal() const
{
    ...
    for (auto PP = PathParser::CreateBegin(__pn_); PP; ++PP) {
        ...
        switch (Kind) {
            ...
            case PK_None: _LIBCPP_UNREACHABLE();
        }
    }
