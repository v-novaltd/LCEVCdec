...
public:
    ...
    void SetUp(benchmark::State& state) override
    {
        ...
        if (!upscaleGetKernel(&ctx, type, &kernel)) {
            state.SkipWithError("Failed to query upscale kernel");
            return;
        }

        surfaceIdle(&ctx, &src);
        if (surfaceInitialise(&ctx, &src, srcFP, srcDimensions.width, srcDimensions.height,
        ...
        surfaceIdle(&ctx, &dst);
        if (surfaceInitialise(&ctx, &dst, dstFP, dstDimensions.width, dstDimensions.height,
...
public:
    ...
    void SetUp(benchmark::State& state) final
    {
        ...
        surfaceIdle(&ctx, &base);
        if (surfaceInitialise(&ctx, &base, srcFP, srcDimensions.width, srcDimensions.height,
                              srcDimensions.width, ILNone)) {
            state.SkipWithError("Failed to initialise source surface");
            return;
        }
...
BENCHMARK_DEFINE_F(UpscaleHoriFixture, UpscaleHorizontal)(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        for (uint32_t y = 0; y < src.height; y += 2) {
            ...
            function(&ctx, srcRows, dstRows, baseRows, src.width, 0, src.width, &kernel, dither);
        }
    }
}
...
BENCHMARK_DEFINE_F(UpscaleFixture, UpscaleVertical)(benchmark::State& state)
{
    ...
    for (auto _ : state) {
        ...
        for (uint32_t x = 0; x < src.width; x += xStep, srcPtr += srcStep, dstPtr += dstStep) {
            function(&ctx, srcPtr, src.stride, dstPtr, dst.stride, 0, src.height, src.height, &kernel);
        }
    }
}
