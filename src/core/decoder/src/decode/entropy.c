                                          ...
                                          HuffmanStream_t* stream, uint8_t* symbolOut)
{
    ...
    if (huffmanLutDecode(&decoder->table, stream, symbolOut) >= 0) {
        return 0;
    }
    if (huffmanManualDecode(&decoder->manualState, stream, symbolOut) >= 0) {
        return 0;
    }
...
#define VN_ENTROPY_DECODE_DEFINE(symbolGetterFn)                                         \
    static int32_t entropyDecode_##symbolGetterFn(EntropyDecoder_t* state, int16_t* out) \
    {                                                                                    \
        int16_t value = 0;                                                               \
        int32_t zeros = 0;                                                               \
        int32_t symbol = 0;                                                              \
...
#define VN_ENTROPY_DECODE_TEMPORAL_DEFINE(symbolGetterFn)                                                 \
    static int32_t entropyDecodeTemporal_##symbolGetterFn(EntropyDecoder_t* state, TemporalSignal_t* out) \
    {                                                                                                     \
...
int32_t entropyDecode(EntropyDecoder_t* state, int16_t* out)
{
