...
void addU8_NEON(const BlitArgs_t* args)
{
...
void addS7_U8_NEON(const BlitArgs_t* args)
{
    const int16x8_t usToSOffset = vdupq_n_s16(16384);
    const int16x8_t fractOffset = vdupq_n_s16(64);
    const int16x8_t signOffset = vdupq_n_s16(128);
...
void addUN_NEON(const BlitArgs_t* args, int32_t shift, int16_t roundingOffset, int16_t signOffset,
                int16_t resultMax, FixedPoint_t unsignedFP)
{
...
void addS16_NEON(const BlitArgs_t* args)
{
    VN_BLIT_SIMD_BOILERPLATE(int16_t, int16_t);
...
static void addU10_NEON(const BlitArgs_t* args) { addUN_NEON(args, 5, 16, 512, 1023, FPU10); }

static void addU12_NEON(const BlitArgs_t* args) { addUN_NEON(args, 3, 4, 2048, 4095, FPU12); }

static void addU14_NEON(const BlitArgs_t* args) { addUN_NEON(args, 1, 1, 8192, 16383, FPU14); }
