...
TEST(BitStream, ReadBit)
{
    ...
    for (uint8_t i = 0; i < numBits; ++i) {
        ...
        uint8_t bit = 0;
        EXPECT_EQ(ldlBitstreamReadBit(&stream, &bit), 0);
        nibble <<= 1;
        nibble |= bit;
    }
    ...
    uint8_t bit;
    EXPECT_EQ(ldlBitstreamReadBit(&stream, &bit), -1);
}
...
#ifdef NDEBUG
    EXPECT_DEATH(ldlBitstreamReadBit(&stream, nullptr), ".*");
#else
    EXPECT_DEATH(ldlBitstreamReadBit(&stream, nullptr), "Assertion .*failed");
#endif
}
